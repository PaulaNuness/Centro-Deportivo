...
public class Conexion_Base_de_Datos {
    ...
    public static ArrayList<String> obteneremailsUsuario() throws SQLException, IOException, ClassNotFoundException {
    ...
    public static boolean insertar_cliente(String email,String nome_Apelido,String tipo,String pago,String falta) {
        boolean ok=false;
        try {
            if(Conexion_Base_de_Datos.email_existe(email)){
                if(!nome_Apelido.equals(" ") &&tipo !=null && pago != null && falta !=null){
                    int penalizacao=0;
                    if(falta =="Si" ){
                        if(tipo =="Premium"){
                            penalizacao=1;
                    ...
                    PreparedStatement pre = a.prepareStatement(sql);
                    pre.setInt(1,Conexion_Base_de_Datos.obtenerIDUsuario(email));
                    pre.setString(2, nome_Apelido);
                    pre.setString(3, tipo);
                    pre.setString(4, pago);
                    pre.setInt(5, penalizacao);
                    pre.executeUpdate();
                    ok=true;
                }
            }else{
                Alert alert = new Alert(Alert.AlertType.INFORMATION);//criar alerta de informacao
                alert.setTitle("Email incorrecto!!!");//com o titulo
                alert.setContentText("Debes eligir un email correcto");
                alert.show();
          }
        } catch (SQLException e) {
            System.out.println(e);
    ...
    public static String obtener_nombre_Usuario(String email) throws SQLException {
        int id=Conexion_Base_de_Datos.obtenerIDUsuario(email);
        String resul=" ";
        try{
            Conexion_Base_de_Datos.conectar();
            Statement s= a.createStatement();
    ...
    public static String obtener_tipo_Usuario(String email) throws SQLException {
        int id=Conexion_Base_de_Datos.obtenerIDUsuario(email);
        String resul=" ";
        try{
            Conexion_Base_de_Datos.conectar();
    ...
    public static String obtener_Pago_Usuario(String email) throws SQLException {
        int id=Conexion_Base_de_Datos.obtenerIDUsuario(email);
        String resul=" ";
        try{
            Conexion_Base_de_Datos.conectar();
    ...
    public static String obtener_Falta_Usuario(String email) throws SQLException {
        int id=Conexion_Base_de_Datos.obtenerIDUsuario(email);
        String resul=" ";
        String resposta=" ";
        try{
    ...
    public static void eliminar_cliente(String email) throws SQLException, IOException, ClassNotFoundException {
        Conexion_Base_de_Datos.conectar();
        Statement s= a.createStatement();
        String sql = ("delete FROM usuarios where email='" + email + "';");
        PreparedStatement sequencia=a.prepareStatement(sql);
        sequencia.executeUpdate();
    }

    public static void actualizar_cliente(String email) throws SQLException, IOException, ClassNotFoundException {
